            ...
            <div className="rounded-2xl p-6 border border-white/50" style={{ background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(16px) saturate(150%)', WebkitBackdropFilter: 'blur(16px) saturate(150%)', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
              <div className="flex justify-between items-start mb-4 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2">
                  {isRestaurantBusiness(selectedBusiness) ? (<div className="px-4 py-2 rounded-lg text-sm font-bold bg-black text-white">Ciro: {formatMoney(calcRestaurantCiro(currentReport))}</div>) : (<div className={`px-4 py-2 rounded-lg text-sm font-bold ${calcCashDiff(currentReport) >= 0 ? 'bg-gray-100 text-gray-900' : 'bg-red-100 text-red-700'}`}>Fark: {formatMoney(calcCashDiff(currentReport))}</div>)}
              ...
              {isRestaurantBusiness(selectedBusiness) ? (<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4"><div className="bg-black text-white p-3 rounded-lg"><p className="font-semibold text-xs flex items-center gap-1 text-gray-300"><Icon path={IconPaths.chart} size={13}/> Toplam Ciro</p><p className="text-lg font-bold">{formatMoney(calcRestaurantCiro(currentReport))}</p><p className="text-[11px] text-gray-400 mt-0.5">Tüm gelirler</p></div><div className="bg-emerald-50 p-3 rounded-lg border border-emerald-200"><p className="text-emerald-600 font-semibold text-xs flex items-center gap-1"><Icon path={IconPaths.cash} size={13}/> Nakit (Adisyo)</p><p className="text-lg font-bold">{formatMoney(currentReport.cash)}</p></div><div className="bg-red-50 p-3 rounded-lg border border-red-200"><p className="text-red-600 font-semibold text-xs flex items-center gap-1"><Icon path={IconPaths.arrowDown} size={13}/> Giderler</p><p className="text-lg font-bold text-red-700">{formatMoney(getExpTotal(currentReport.expenses))}</p></div><div className={`p-3 rounded-lg border ${calcRestaurantCashOnHand(currentReport) < 0 ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}><p className={`font-semibold text-xs flex items-center gap-1 ${calcRestaurantCashOnHand(currentReport) < 0 ? 'text-red-600' : 'text-gray-600'}`}><Icon path={IconPaths.wallet} size={13}/> Eldeki Nakit{calcRestaurantCashOnHand(currentReport) < 0 && <span className="ml-1 text-[10px] bg-red-200 text-red-800 px-1.5 py-0.5 rounded-full">açık</span>}</p><p className={`text-lg font-bold ${calcRestaurantCashOnHand(currentReport) < 0 ? 'text-red-700' : ''}`}>{formatMoney(calcRestaurantCashOnHand(currentReport))}</p><p className="text-[11px] text-gray-400 mt-0.5">Nakit − Giderler</p></div></div>) : (<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
