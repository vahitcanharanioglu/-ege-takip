...
export default function App() {
  ...
  // Geçmiş giderlerde personel adı taraması
  const [scanModal, setScanModal] = useState(null); // { rows: [...], loading, applying }
  ...
  const handleAddExpense = () => {
    ...
    setNewExpense({ description: '', amount: '', employee_id: '', is_external: false, employeeManual: false, autoDetected: false });
  };
  ...
  // ---- PERSONEL ADI OTOMATİK ALGILAMA (yazım hatası toleranslı) ----
  // Türkçe karakterleri sadeleştirir: İ/ı→i, Ğ→g, Ü→u, Ş→s, Ö→o, Ç→c
  ...
  const isMesaiText = (text) => {
    ...
    return /\bmesai\w*\b/.test(n) || /\bfazla mesai\b/.test(n);
  };

  // Açıklama metninde personel adı ara; en iyi eşleşen personeli döndür
  ...
  const handleExpenseDescriptionChange = (value) => {
    setNewExpense(prev => {
      ...
      if (!prev.employeeManual) {
        const detected = detectEmployeeFromText(value);
        next.employee_id = detected || '';
  ...
  const allocateSalaryFIFOWith = (emp, amount, paymentsForEmp, extraPaid = {}) => {
    ...
    for (const p of SALARY_PERIOD) {
      ...
      if (rem > 0) {
        ...
        parts.push({ year: p.year, month: p.month, amount: Math.round(take * 100) / 100 });
    ...
    if (left > 0.001) {
      ...
      if (ex) ex.amount = Math.round((ex.amount + left) * 100) / 100;
      else parts.push({ year: cur.year, month: cur.month, amount: Math.round(left * 100) / 100 });
    }
    // Yuvarlama sonrası toplam, gider tutarıyla birebir eşit olmalı (kuruş kaybı önlenir)
    if (parts.length > 0) {
  ...
  const runEmployeeScan = async () => {
    ...
    try {
      ...
      setScanModal({ rows: [], loading: false, applying: false, error: e.message });
    }
  };

  const toggleScanRow = (id) => {
    setScanModal(m => m ? { ...m, rows: m.rows.map(r => r.id === id ? { ...r, selected: !r.selected } : r) } : m);
  };

  // Satırdaki personeli elle değiştir (algoritma yanlış bulduysa veya hiç bulamadıysa)
  const setScanRowEmployee = (id, empId) => {
    setScanModal(m => m ? { ...m, rows: m.rows.map(r => r.id === id
      ? { ...r, employee_id: empId, employee_name: empId ? getEmployeeName(empId) : '', selected: !!empId }
      : r) } : m);
  };

  // Seçili satırları uygula: gideri personele ata + maaş ödemesi oluştur
  const applyEmployeeScan = async () => {
    if (!scanModal) return;
    const picked = scanModal.rows.filter(r => r.selected && r.employee_id);
    if (picked.length === 0) return;
    setScanModal(m => ({ ...m, applying: true }));
    try {
      // 1) Giderleri personele ata
      ...
      setScanModal(m => ({ ...m, applying: false, error: 'Uygulanamadı: ' + e.message }));
    }
  };
  ...
  const fetchAdisyoDay = async (dateStr) => {
    ...
    try {
      ...
      // Restoran mantığı: Adisyo'nun TÜM kalemleri gelirlerde ayrı ayrı görünür (adisyoPayments).
      // Kasa hesabı SADECE nakite bağlıdır → yalnızca "Nakit" kalemi cash alanına yansır.
      // Kart/online/yemek kartı kalemleri kasa farkına KARIŞMAZ; ayrı kalem olarak saklanır.
      let cash = 0;
      for (const r of rows) {
        if (r.is_debit) continue; // veresiye kasaya girmez
        if (r.is_meal_card) continue; // yemek kartı nakit değil
        const nm = r.payment_name.toLocaleLowerCase('tr-TR');
        if (nm.includes('nakit')) cash += r.amount;
      }
      const fmt = (n) => (Math.round(n * 100) / 100).toString();
      // Sadece nakit alanını doldur; credit_card/meal_cards'a dokunma (kalemler ayrı saklanıyor)
      setReportForm(f => ({ ...f, date: dateStr, cash: fmt(cash) }));
      setAdisyoProgress('');
    } catch (e) {
  ...
  // Adisyo kaleminin tutarını elle düzenle (index bazlı)
  // Nakit kalemi değişirse cash alanı (kasa hesabı) da güncellenir.
  const updateAdisyoPaymentAmount = (index, newValue) => {
    setAdisyoPayments(prev => {
      const next = prev.map((p, i) => i === index ? { ...p, amount: newValue === '' ? 0 : (parseFloat(String(newValue).replace(',', '.')) || 0) } : p);
      // Nakit toplamını yeniden hesapla → cash alanına yansıt
      let cash = 0;
      for (const r of next) {
        if (r.is_debit || r.is_meal_card) continue;
        if ((r.payment_name || '').toLocaleLowerCase('tr-TR').includes('nakit')) cash += Number(r.amount) || 0;
      }
      setReportForm(f => ({ ...f, cash: (Math.round(cash * 100) / 100).toString() }));
      return next;
    });
  };

  // Restoranda ana "Nakit" alanı elle değiştirilirse, gelir listesindeki Nakit kalemini de güncelle
  // (yoksa Toplam Ciro eski değerde kalır)
  const setRestaurantCash = (value) => {
    setReportForm(f => ({ ...f, cash: value }));
    const num = value === '' ? 0 : (parseFloat(String(value).replace(',', '.')) || 0);
    setAdisyoPayments(prev => {
      if (prev.length === 0) return prev;
      const idx = prev.findIndex(p => !p.is_debit && !p.is_meal_card && (p.payment_name || '').toLocaleLowerCase('tr-TR').includes('nakit'));
      if (idx === -1) return prev;
      return prev.map((p, i) => i === idx ? { ...p, amount: num } : p);
    });
  };

  // Adisyo kalemini listeden çıkar
  const removeAdisyoPayment = (index) => {
    setAdisyoPayments(prev => {
  ...
  // Restoran: eldeki nakit = Adisyo Nakit − nakit giderleri (otomatik hesap)
  const calcRestaurantCashOnHand = (r) => Number(r.cash || 0) - getExpTotal(r.expenses);
  // Restoran: toplam ciro = Adisyo tüm kalemleri (veresiye hariç). Kalem yoksa 3 ana alan toplamı.
  const calcRestaurantCiro = (r) => {
    const pays = r.daily_report_payments || [];
    if (pays.length > 0) return pays.filter(p => !p.is_debit).reduce((s, p) => s + Number(p.amount), 0);
    return Number(r.credit_card || 0) + Number(r.cash || 0) + Number(r.meal_cards || 0);
  };
          ...
          {currentReport ? (
            <div className="rounded-2xl p-6 border border-white/50" style={{ background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(16px) saturate(150%)', WebkitBackdropFilter: 'blur(16px) saturate(150%)', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
              <div className="flex justify-between items-start mb-4 flex-wrap gap-2">
                <div><p className="text-2xl font-bold">{formatDateTR(currentReport.date)}</p><p className="text-sm text-gray-500">Ciro: {formatMoney(isRestaurantBusiness(selectedBusiness) ? calcRestaurantCiro(currentReport) : (Number(currentReport.credit_card) + Number(currentReport.cash) + Number(currentReport.meal_cards)))}</p><p className="text-xs text-black mt-1">{currentReport.fullName} - {formatTimeTR(currentReport.created_at)}</p>{currentReport.updated_by_name && <p className="text-xs text-amber-600 mt-0.5 flex items-center gap-1"><Icon path={IconPaths.edit} size={11}/> Düzenleyen: {currentReport.updated_by_name}{currentReport.updated_at ? ` · ${formatDateTimeTR(currentReport.updated_at)}` : ''}</p>}</div>
                <div className="flex items-center gap-2">
                  {isRestaurantBusiness(selectedBusiness) ? (<div className="px-4 py-2 rounded-lg text-sm font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">Ciro: {formatMoney(calcRestaurantCiro(currentReport))}</div>) : (<div className={`px-4 py-2 rounded-lg text-sm font-bold ${calcCashDiff(currentReport) >= 0 ? 'bg-gray-100 text-gray-900' : 'bg-red-100 text-red-700'}`}>Fark: {formatMoney(calcCashDiff(currentReport))}</div>)}
              ...
              {isRestaurantBusiness(selectedBusiness) ? (<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4"><div className="bg-emerald-50 p-3 rounded-lg border border-emerald-200"><p className="font-semibold text-xs flex items-center gap-1 text-emerald-600"><Icon path={IconPaths.chart} size={13}/> Toplam Ciro</p><p className="text-lg font-bold text-emerald-700">{formatMoney(calcRestaurantCiro(currentReport))}</p><p className="text-[11px] text-gray-400 mt-0.5">Tüm gelirler</p></div><div className="bg-emerald-50 p-3 rounded-lg border border-emerald-200"><p className="text-emerald-600 font-semibold text-xs flex items-center gap-1"><Icon path={IconPaths.cash} size={13}/> Nakit (Adisyo)</p><p className="text-lg font-bold">{formatMoney(currentReport.cash)}</p></div><div className="bg-red-50 p-3 rounded-lg border border-red-200"><p className="text-red-600 font-semibold text-xs flex items-center gap-1"><Icon path={IconPaths.arrowDown} size={13}/> Giderler</p><p className="text-lg font-bold text-red-700">{formatMoney(getExpTotal(currentReport.expenses))}</p></div><div className={`p-3 rounded-lg border ${calcRestaurantCashOnHand(currentReport) < 0 ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'}`}><p className={`font-semibold text-xs flex items-center gap-1 ${calcRestaurantCashOnHand(currentReport) < 0 ? 'text-red-600' : 'text-emerald-600'}`}><Icon path={IconPaths.wallet} size={13}/> Eldeki Nakit{calcRestaurantCashOnHand(currentReport) < 0 && <span className="ml-1 text-[10px] bg-red-200 text-red-800 px-1.5 py-0.5 rounded-full">açık</span>}</p><p className={`text-lg font-bold ${calcRestaurantCashOnHand(currentReport) < 0 ? 'text-red-700' : 'text-emerald-700'}`}>{formatMoney(calcRestaurantCashOnHand(currentReport))}</p><p className="text-[11px] text-gray-400 mt-0.5">Nakit − Giderler</p></div></div>) : (<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              ...
              {(currentReport.daily_report_payments || []).length > 0 && (<div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-4"><p className="text-gray-700 font-semibold text-sm mb-2">Adisyo Ödeme Türleri (Gelirler)</p>{currentReport.daily_report_payments.slice().sort((a,b)=>b.amount-a.amount).map((p, i) => (<div key={i} className="flex justify-between text-sm bg-white p-2 rounded mb-1 border border-gray-100"><span className="text-gray-700">{p.payment_name}{p.is_meal_card && <span className="ml-1.5 text-xs bg-gray-200 text-gray-600 px-1.5 py-0.5 rounded-full">yemek kartı</span>}{p.is_debit && <span className="ml-1.5 text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full">veresiye</span>}</span><span className="font-semibold text-gray-900">{formatMoney(p.amount)}</span></div>))}<div className="flex justify-between pt-1 text-sm"><span className="font-semibold text-gray-600">Toplam:</span><span className="font-bold text-gray-900">{formatMoney(currentReport.daily_report_payments.reduce((s,p)=>s+Number(p.amount),0))}</span></div></div>)}
    ...
    return (
      <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #f5f5f7 0%, #e8e8ec 50%, #dddde3 100%)' }}>
        ...
        {scanModal && (<div className="fixed inset-0 bg-black/50 flex items-start justify-center p-4 z-50 overflow-y-auto"><div className="rounded-2xl p-6 w-full max-w-2xl my-8 max-h-[90vh] overflow-y-auto border border-white/60" style={{ background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
          ...
          {!scanModal.loading && scanModal.rows.length > 0 && (<>
            <div className="flex gap-2 mb-3"><button onClick={() => setScanModal(m => ({...m, showUnmatched: false}))} className={`px-3 py-1.5 rounded-lg text-sm font-semibold ${!scanModal.showUnmatched ? 'bg-black text-white' : 'bg-black/5 text-gray-700'}`}>Bulunanlar ({scanModal.rows.filter(r=>r.matched).length})</button><button onClick={() => setScanModal(m => ({...m, showUnmatched: true}))} className={`px-3 py-1.5 rounded-lg text-sm font-semibold ${scanModal.showUnmatched ? 'bg-black text-white' : 'bg-black/5 text-gray-700'}`}>Eşleşmeyenler ({scanModal.rows.filter(r=>!r.matched).length})</button></div>
            {scanModal.showUnmatched && <p className="text-xs text-gray-500 mb-2">Adı algılanamayan giderler. Personele aitse listeden seç; değilse boş bırak.</p>}
            <div className="flex justify-between items-center mb-2 text-sm"><span className="text-gray-600">{scanModal.rows.filter(r=>r.selected && r.employee_id).length} kayıt atanacak</span><span className="font-bold text-gray-900">Toplam: {formatMoney(scanModal.rows.filter(r=>r.selected && r.employee_id).reduce((s,r)=>s+r.amount,0))}</span></div>
            <div className="max-h-80 overflow-y-auto space-y-1.5 mb-4">{scanModal.rows.filter(r => scanModal.showUnmatched ? !r.matched : r.matched).map(r => (
              <div key={r.id} className={`p-2 rounded-lg border ${r.selected && r.employee_id ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-gray-200'}`}>
                <div className="flex items-center gap-2">
                  <input type="checkbox" checked={r.selected} disabled={!r.employee_id} onChange={() => toggleScanRow(r.id)} className="w-4 h-4 accent-black disabled:opacity-30" />
                  <span className="flex-1 min-w-0"><span className="text-sm text-gray-800">{r.description}</span>{r.is_external && <span className="ml-1.5 text-[10px] bg-gray-600 text-white px-1.5 py-0.5 rounded-full">havale</span>}<span className="block text-xs text-gray-500">{r.date ? formatDateTR(r.date) : '—'}</span></span>
                  <span className="text-sm font-bold text-red-600 whitespace-nowrap">{formatMoney(r.amount)}</span>
                </div>
                <select value={r.employee_id} onChange={(e) => setScanRowEmployee(r.id, e.target.value)} className="w-full mt-1.5 px-2 py-1 border border-gray-200 rounded-lg text-xs bg-white text-gray-700 focus:border-black focus:outline-none"><option value="">— personele atama —</option>{getActiveEmployees().map(emp => (<option key={emp.id} value={emp.id}>{emp.name}</option>))}</select>
              </div>))}
            </div>
          </>)}
          <div className="flex gap-2"><button onClick={() => setScanModal(null)} className="flex-1 bg-black/5 text-gray-700 py-3 rounded-xl font-semibold hover:bg-black/10 transition">Kapat</button>{scanModal.rows.length > 0 && (<button onClick={applyEmployeeScan} disabled={scanModal.applying || scanModal.rows.filter(r=>r.selected).length===0} className="flex-1 bg-black text-white py-3 rounded-xl font-semibold hover:bg-gray-800 transition disabled:opacity-50">{scanModal.applying ? 'Uygulanıyor…' : 'Seçilenleri Ata'}</button>)}</div>
        </div></div>)}
      </div>
    );
  }
