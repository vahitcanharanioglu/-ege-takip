          ...
          {currentReport ? (
            <div className="rounded-2xl p-6 border border-white/50" style={{ background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(16px) saturate(150%)', WebkitBackdropFilter: 'blur(16px) saturate(150%)', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
              <div className="flex justify-between items-start mb-4 flex-wrap gap-2">
                <div><p className="text-2xl font-bold">{formatDateTR(currentReport.date)}</p><p className="text-sm text-gray-500">Ciro: {formatMoney(isRestaurantBusiness(selectedBusiness) && (currentReport.daily_report_payments || []).length > 0 ? currentReport.daily_report_payments.filter(p => !p.is_debit).reduce((s,p)=>s+Number(p.amount),0) : (Number(currentReport.credit_card) + Number(currentReport.cash) + Number(currentReport.meal_cards)))}</p><p className="text-xs text-black mt-1">{currentReport.fullName} - {formatTimeTR(currentReport.created_at)}</p>{currentReport.updated_by_name && <p className="text-xs text-amber-600 mt-0.5 flex items-center gap-1"><Icon path={IconPaths.edit} size={11}/> Düzenleyen: {currentReport.updated_by_name}{currentReport.updated_at ? ` · ${formatDateTimeTR(currentReport.updated_at)}` : ''}</p>}</div>
              ...
              {isRestaurantBusiness(selectedBusiness) ? (<div className="grid grid-cols-2 gap-3 mb-4"><div className="bg-emerald-50 p-3 rounded-lg border border-emerald-200"><p className="text-emerald-600 font-semibold text-xs flex items-center gap-1"><Icon path={IconPaths.cash} size={13}/> Nakit (kasa)</p><p className="text-lg font-bold">{formatMoney(currentReport.cash)}</p></div><div className="bg-gray-50 p-3 rounded-lg border border-gray-200"><p className="text-gray-600 font-semibold text-xs flex items-center gap-1"><Icon path={IconPaths.wallet} size={13}/> Eldeki Nakit</p><p className="text-lg font-bold">{formatMoney(currentReport.actual_cash)}</p></div></div>) : (<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
