        ...
        <main className="max-w-4xl mx-auto px-4 py-6">
          ...
          {currentReport ? (
            <div className="rounded-2xl p-6 border border-white/50" style={{ background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(16px) saturate(150%)', WebkitBackdropFilter: 'blur(16px) saturate(150%)', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
              ...
              {isRestaurantBusiness(selectedBusiness) ? (<div className="grid grid-cols-2 gap-3 mb-4"><div className="bg-emerald-50 p-3 rounded-lg border border-emerald-200"><p className="text-emerald-600 font-semibold text-xs flex items-center gap-1"><Icon path={IconPaths.cash} size={13}/> Nakit (Adisyo)</p><p className="text-lg font-bold">{formatMoney(currentReport.cash)}</p></div><div className="bg-gray-50 p-3 rounded-lg border border-gray-200"><p className="text-gray-600 font-semibold text-xs flex items-center gap-1"><Icon path={IconPaths.wallet} size={13}/> Eldeki Nakit</p><p className="text-lg font-bold">{formatMoney(calcRestaurantCashOnHand(currentReport))}</p><p className="text-[11px] text-gray-400 mt-0.5">Nakit − Giderler</p></div></div>) : (<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
